...
namespace Gala {
    ...
    internal static void make_background (Wl.Client client, Wl.Resource resource, int monitor_index) {
        unowned ExtendedBehaviorSurface? eb_surface = resource.get_user_data<ExtendedBehaviorSurface> ();
        if (eb_surface.wayland_surface == null) {
            return;
        }
        ...
        ShellClientsManager.get_instance ().make_background (window, monitor_index);
    }
