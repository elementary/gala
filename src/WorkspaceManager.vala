...
namespace Gala {
    public class WorkspaceManager : Object {
        ...
        private void workspace_switched (Meta.WorkspaceManager manager, int from, int to, MotionDirection direction) {
            if (!Prefs.get_dynamic_workspaces ())
                return;
...
#else
        private void workspace_added (Screen screen, int index) {
        ...
        private void workspace_removed (Screen screen, int index) {
        ...
        private void workspace_switched (Screen screen, int from, int to, MotionDirection direction) {
            if (!Prefs.get_dynamic_workspaces ())
                return;
        ...
        private void window_added (Workspace? workspace, Window window) {
        ...
        private void window_removed (Workspace? workspace, Window window) {
...
#if HAS_MUTTER330
        private void window_entered_monitor (Meta.Display display, int monitor, Window window) {
            if (InternalUtils.workspaces_only_on_primary ()
                && monitor == display.get_primary_monitor ())
                window_added (window.get_workspace (), window);
        }

        private void window_left_monitor (Meta.Display display, int monitor, Window window) {
            if (InternalUtils.workspaces_only_on_primary ()
                && monitor == display.get_primary_monitor ())
                window_removed (window.get_workspace (), window);
        }
#else
        private void window_entered_monitor (Screen screen, int monitor, Window window) {
            if (InternalUtils.workspaces_only_on_primary ()
                && monitor == screen.get_primary_monitor ())
                window_added (window.get_workspace (), window);
        }

        private void window_left_monitor (Screen screen, int monitor, Window window) {
            if (InternalUtils.workspaces_only_on_primary ()
                && monitor == screen.get_primary_monitor ())
                window_removed (window.get_workspace (), window);
        }
         ...
         */
        private void remove_workspace (Workspace workspace) {
#if HAS_MUTTER330
