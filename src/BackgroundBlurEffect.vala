 ...
 */
 ...
 public class Gala.BackgroundBlurEffect : Clutter.Effect {
    ...
    private float real_clip_radius;
    ...
    private void update_clip_radius () {
        /* we need to scale twice because we compensate both new "real clip radius" -
         * e.g. when scale = 2 we need to make clip_radius itself 2 times larger
         * but also since we're working with shader we need to compensate for pixel step,
         * where we divide by monitor_scale.
         */
        float[] _clip_radius = { clip_radius * monitor_scale * monitor_scale };
    ...
    public override void paint_node (Clutter.PaintNode node, Clutter.PaintContext paint_context, Clutter.EffectPaintFlags flags) {
        ...
        real_blur_radius = blur_radius * (1.0f - (float) (255 - actor.opacity) / 255.0f) * monitor_scale;
        real_clip_radius = clip_radius * monitor_scale;
