...
namespace Gala {
    ...
    public class WorkspaceClone : ActorTarget {
        ...
        private void update_icon_group_opacity () {
            var offset = (workspace.index () * width + get_parent ().x).abs ();
            icon_group.backdrop_opacity = (1 - (offset / width)).clamp (0, 1);
        }
        ...
        private void update_targets () {
            ...
            var target_width = monitor.width * scale + WindowManagerGala.WORKSPACE_GAP;

            add_target (new PropertyTarget (MultitaskingView.GESTURE_ID, this, "width", typeof (float), (float) initial_width, (float) target_width));
            add_target (new PropertyTarget (MultitaskingView.GESTURE_ID, background, "scale-x", typeof (double), 1d, (double) scale));
            add_target (new PropertyTarget (MultitaskingView.GESTURE_ID, background, "scale-y", typeof (double), 1d, (double) scale));
