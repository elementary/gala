...
public class Gala.IconGroup : CanvasActor {
    ...
    private GLib.List<Meta.Window> windows_list = new GLib.List<Meta.Window> ();
...
//FIXME: TODO!
    ...
    public void add_window (Meta.Window window, bool no_redraw = false, bool temporary = false) {
        if (!windows_list.find (window).is_empty ()) {
            return;
        }
    ...
    public void remove_all_windows () {
        windows_list = new GLib.List<Meta.Window> ();
        icon_container.remove_all_children ();
    }
